...
9. **Approval Status** - APPROVE, REQUEST_CHANGES, or COMMENT
...
Format with clear sections and provide specific examples."""
    ...
    def post_review(
        ...
        GitHub does not allow a user to APPROVE or REQUEST_CHANGES on their
        own pull request. If the API returns a 422 error (e.g. because the
        token owner authored the PR), we retry with event="COMMENT" so the
        review is still posted as a proper PR review rather than falling back
        to a plain issue comment.

        Args:
            pr_number: PR number
            review_text: Review content
            approval_status: Approval status (APPROVE, REQUEST_CHANGES, COMMENT)
        ...
        try:
            ...
            # If we tried APPROVE or REQUEST_CHANGES and got a 422, it's
            # likely because the token owner is also the PR author. GitHub
            # doesn't allow self-approval. Retry as a plain COMMENT review.
            if "422" in str(e) and approval_status != "COMMENT":
                print_step(
                    f"Cannot post review as '{approval_status}' (the token "
                    f"owner may be the PR author). Retrying as COMMENT..."
                )
                try:
                    body_with_status = (
                        f"**Review Status: {approval_status}**\n\n"
                        f"{review_text}"
                    )
                    review_response = self.github.create_pr_review(
                        pr_number=pr_number,
                    ...
                    print_success("Review posted as COMMENT (with status in body)")
                    return review_response
                except Exception as retry_err:
                    print_error(f"Retry as COMMENT also failed: {str(retry_err)}")
...
"""
                ...
                print_success("Review posted as issue comment (fallback)")
